  ...
  const changeUserStatusApi = async (id: number, status: boolean) => {
    const URL_API = `/api/user/changestatus/${id}?status=${status}`;
    const response = await defaultAxiosInstance.post(URL_API);
    return response.data;
  };
  ...
  export { createUserApi, loginUserApi, getCurrentLogin, getAllUserApi, changeUserRoleApi, changeUserStatusApi, deleteUserApi, updateAccountApi, createAdminUserApi, getUserByIdApi, updateUserByIdApi, verifyUserByIdApi, createStaffApi, createManagerApi, googleSignUpApi, googleSigInpApi };
